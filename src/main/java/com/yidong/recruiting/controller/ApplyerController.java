...
public class ApplyerController {
    ...
    public String enter(
            ...
            @RequestParam("telephone")String iphone,
        ...
        if(applyerService.judgeEntry(telephone, num).isEmpty()){
            applyerService.entry(telephone, num,iphone,user.getUserName(),position);
    ...
    public String mine( Model model){
        ...
        Session session = subject.getSession();
        User user = (User)session.getAttribute("user");
        List<Apply> applyList = applyerService.myInterview(user.getTelephone());
    ...
    public String noQueue(
        ...
        applyerService.noQueue(telephone,position);
        Subject subject = SecurityUtils.getSubject();
        Session session = subject.getSession();
        User user = (User)session.getAttribute("user");
        List<Apply> applyList = applyerService.myInterview(user.getTelephone());
    ...
    public String toQueue(
        ...
        applyerService.toQueue(telephone,position);
        Subject subject = SecurityUtils.getSubject();
        Session session = subject.getSession();
        User user = (User)session.getAttribute("user");
        List<Apply> applyList = applyerService.myInterview(user.getTelephone());
